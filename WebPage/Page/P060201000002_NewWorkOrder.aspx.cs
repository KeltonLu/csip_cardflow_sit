...
public partial class P06020101_NewWorkOrder : PageBase
{
    ...
    protected void btnBack_Click(object sender, EventArgs e)
    {
        ...
        else
        {
                ...
                Request.QueryString["searchMailNo"] != null)
            {
                ...
                Response.Redirect("P060201000001_NewWorkOrder.aspx?searchID=" + RedirectHelper.GetEncryptParam(strID) + "&searchNo=" + RedirectHelper.GetEncryptParam(strNo) + "&searchMailNo=" + RedirectHelper.GetEncryptParam(strMailNo) + "", false);
            }
            else
            {
                Response.Redirect("P060201000001_NewWorkOrder.aspx");
            }
        }
    }
    ...
    protected void btnSureS_Click(object sender, EventArgs e)
    {
        ...
        esborderup.CDM_C0701_MEMO = this.txtRemark_dispatch.Text.Trim();
        ...
        //CardDataChange.NoteCaptions = String.Format("{0},{1},{2}", ((CSIPCommonModel.EntityLayer.EntityAGENT_INFO)Session["Agent"]).agent_id, DateTime.Now.ToString("yyyy/MM/dd"), "發送工單通知客服");//*異動記錄說明
        CardDataChange.UpdDate = DateTime.Now.ToString("yyyy/MM/dd");
        CardDataChange.UpdTime = DateTime.Now.ToString("HH:mm");
        CardDataChange.UpdUser = ((CSIPCommonModel.EntityLayer.EntityAGENT_INFO)Session["Agent"]).agent_id;
        CardDataChange.CardNo = m_CardNo;
