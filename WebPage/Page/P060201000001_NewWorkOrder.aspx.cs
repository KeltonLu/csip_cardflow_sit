...
public partial class P060201000001_NewWorkOrder : PageBase
{
    ...
    protected void grvUserView_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            //* 傳遞參數加密
            Response.Redirect("P060201000002_NewWorkOrder.aspx?action=" + RedirectHelper.GetEncryptParam(strAction) + " &id=" + RedirectHelper.GetEncryptParam(strId) + "&cardno=" + RedirectHelper.GetEncryptParam(strCardNo) + "&trandate=" + RedirectHelper.GetEncryptParam(strTrandate) + "&searchID=" + RedirectHelper.GetEncryptParam(txtId.Text) + "&searchNo="+RedirectHelper.GetEncryptParam(txtNo.Text) + "&searchMailNo="+RedirectHelper.GetEncryptParam(txtMailNo.Text) + "", false);
        }
        catch (Exception exp)
        {
            Logging.Log(exp);
