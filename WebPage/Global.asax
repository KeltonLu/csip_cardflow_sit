...
<script RunAt="server">
	void Application_Start(object sender, EventArgs e)
	{
		...
		#region 系統框架啟動
		// 應用程式啟動時執行的程式碼
		if (!CSIPCommonModel.BaseItem.PopedomManager.MainPopedomManager.Inited)
		{
			CSIPCommonModel.BaseItem.PopedomManager.MainPopedomManager.Init();
		}
		#endregion
		#region Job啟動      
		string strMsgID = "";
		try
		{
			...
			System.Data.DataTable dtblAutoJob = CSIPCommonModel.BusinessRules.BRM_AUTOJOB.GetJobData(ConfigurationManager.AppSettings["FunctionKey"].ToString(), ref strMsgID);
			if (dtblAutoJob == null)
			{
				//* 取得Job資料失敗
	...
	void Application_End(object sender, EventArgs e)
	{
		...
		if (sched != null)
		{
			...
			//* 訪問系統頁面，以重新啟動JOB排程
			System.Net.HttpWebRequest request = (System.Net.HttpWebRequest)System.Net.WebRequest.Create(ConfigurationManager.AppSettings["LOGOUT"].ToString());
